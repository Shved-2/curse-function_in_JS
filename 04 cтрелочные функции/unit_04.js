...
const t9 = (arg) => {
  return arg.reduce((acum, item) => (acum += item));
};
...
const t15 = (num) => !isNaN(Number(num));
...
const t16 = (...arguments) => arguments[0].length % 2 === 0;
...
const t17 = (...arg) => {
  let bool = arg[0].filter((item) => typeof item === 'number');
  return bool.length == arg[0].length;
};

document.querySelector('.b-17').addEventListener('click', function () {
  document.querySelector('.out-17').textContent = t17([4, 5, 6, 'fghf']);
});
...
const t18 = (arr, symb) => arr.includes(symb);
